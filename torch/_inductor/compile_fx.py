...
def _resolve_name_collision(mod: GraphModule, gm: GraphModule) -> None:
    ...
    We resolve this potential name collision here by adding "_" to the end of
    the target names with a collision.
    """
    for node in gm.graph.nodes:
        if node.op == "get_attr":
            if not node.target.startswith(
                "_tensor_constant"
            ) and not node.target.startswith("_torchbind_obj"):
                continue
            old_target_name = node.target
            new_target_name = old_target_name
            while hasattr(mod, new_target_name):
                gm_target = attrgetter(new_target_name)(gm)
                model_target = attrgetter(new_target_name)(mod)
                if (
                    not torch.equal(gm_target, model_target)
                ) or gm_target.dtype != model_target.dtype:
                    new_target_name += "_"
                else:
                    break
            if old_target_name != new_target_name:
                node.target = new_target_name
                setattr(gm, new_target_name, attrgetter(old_target_name)(gm))
